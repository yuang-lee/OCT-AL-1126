#!/usr/bin/env bash
# 學習曲線（seed 10）— SimCLR_1(θ¹: random→SimCLR) / SimCLR_2(θ²: ImageNet→SimCLR)，portion 10/30（ρ=100 不畫）。
...
# 用法：DEVICE=cuda:8 ./thesis/chapter_4/run_4_3_learning_curve_simclr_s10.sh
set -e
cd "$(dirname "$0")/../.."          # repo 根
...
# SimCLR_1（θ¹: random→SimCLR；seed10 best lr）
run simclr1 random   10  5e-5
run simclr1 random   30  5e-5

# SimCLR_2（θ²: ImageNet→SimCLR；seed10 best lr）
run simclr2 imagenet 10  1e-4
run simclr2 imagenet 30  5e-5

echo "✓ simclr (seed10) done -> $OUT/{simclr1,simclr2}_p{10,30}_s10.json"
