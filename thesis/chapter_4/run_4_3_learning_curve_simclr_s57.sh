#!/usr/bin/env bash
# 學習曲線（seed 57）— SimCLR_1(θ¹: random→SimCLR) / SimCLR_2(θ²: ImageNet→SimCLR)，portion 10/30。
...
# 用法：DEVICE=cuda:7 ./thesis/chapter_4/run_4_3_learning_curve_simclr_s57.sh
set -e
cd "$(dirname "$0")/../.."          # repo 根
...
# SimCLR_1（θ¹: random→SimCLR；seed57 best lr）
run simclr1 random   10  3e-5
run simclr1 random   30  1e-4

# SimCLR_2（θ²: ImageNet→SimCLR；seed57 best lr）
run simclr2 imagenet 10  1e-4
run simclr2 imagenet 30  1e-4

echo "✓ simclr (seed57) done -> $OUT/{simclr1,simclr2}_p{10,30}_s57.json"
