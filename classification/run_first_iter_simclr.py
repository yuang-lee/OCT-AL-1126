...
def parse_arguments():
    ...
    # SimCLR pretraining checkpoint configs. These are used only when --pretrained_weights simclr.
    parser.add_argument('--simclr_lr', type=str, default='0.0002')
...
def main():
    ...
    # Generate file name
    # bs/ep here refer to the downstream supervised training config.
    file_name = f"random{args.seed}_bs{args.batch_size}_ep{args.epoch}"
    if args.pretrained_weights == 'simclr':
        # Explicitly record the SimCLR pretraining config in the result JSON filename.
        file_name += f"_simclr_lr{args.simclr_lr}_simclr_bs{args.simclr_bs}_simclr_ep{args.simclr_ep}"
    ...
    if args.pretrained_weights == 'random':
        ...
        model, simclr_path = initialize_simclr_model(
            num_classes,
            args.simclr_lr,
            args.simclr_bs,
            args.simclr_ep,
        ...
        print(f'SimCLR model path: {simclr_path}')
